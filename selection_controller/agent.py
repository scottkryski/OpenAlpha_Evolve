...
class SelectionControllerAgent(SelectionControllerInterface, BaseAgent):
    ...
    def select_parents(self, population: List[Program], num_parents: int) -> List[Program]:
        ...
        sorted_population = sorted(
            population,
            key=lambda p: (
                p.fitness_scores.get("correctness_score", 0.0),
        ...
        total_fitness = sum(p.fitness_scores.get("correctness_score", 0.0) + 0.0001 for p in roulette_candidates)
        ...
        if total_fitness <= 0.0001 * len(roulette_candidates): # All effectively zero
            ...
            for _ in range(remaining_slots):
                ...
                for program in roulette_candidates:
                    current_sum += (program.fitness_scores.get("correctness_score", 0.0) + 0.0001)
                    if current_sum >= pick:
                ...
                if chosen_parent:
                    ...
                    logger.debug(f"Selected parent via roulette: {chosen_parent.id} (Fitness: {chosen_parent.fitness_scores.get('correctness_score')})")
                else:
    ...
    def select_survivors(self, current_population: List[Program], offspring_population: List[Program], population_size: int) -> List[Program]:
        ...
        for program in sorted_combined:
            if len(survivors) < population_size:
                ...
                # Here, we assume program.id is the unique identifier for a program version.
