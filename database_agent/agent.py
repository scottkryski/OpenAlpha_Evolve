...
class InMemoryDatabaseAgent(DatabaseAgentInterface, BaseAgent):
    ...
    async def save_program(self, program: Program) -> None:
        ...
        logger.debug(f"Program {program.id} data: {program}") # Could be verbose
    ...
    async def get_best_programs(
        self,
        task_id: str, # Added task_id to match interface
        limit: int = 5,
        objective: Literal["correctness_score", "runtime_ms"] = "correctness_score",
        sort_order: Literal["asc", "desc"] = "desc",
    ) -> List[Program]:
        # Log that task_id is received but not used for filtering in this specific implementation
        if task_id:
            logger.debug(f"InMemoryDatabaseAgent.get_best_programs called with task_id='{task_id}', but this implementation does not currently filter by task_id for selecting best programs across all tasks.")
        ...
        def sort_key(p: Program):
            if objective == "correctness_score":
                return p.fitness_scores.get("correctness_score", -1.0) # Default to low score if missing
        ...
        if objective == "runtime_ms" and sort_order == "asc":
            ...
            sorted_programs = sorted(all_progs, key=lambda p: p.fitness_scores.get("runtime_ms", float('inf')), reverse=False)
        elif objective == "runtime_ms" and sort_order == "desc":
            # Sort by negative runtime, as higher (less negative) is better and reverse=True
             sorted_programs = sorted(all_progs, key=lambda p: -p.fitness_scores.get("runtime_ms", float('-inf')), reverse=False) # -inf makes it sort correctly when negated for desc
        elif objective == "correctness_score":
            sorted_programs = sorted(all_progs, key=lambda p: p.fitness_scores.get("correctness_score", -1.0), reverse=effective_reverse)
        else:
    ...
    async def get_programs_for_next_generation(self, task_id: str, generation_size: int) -> List[Program]:
        ...
        # Note: task_id is not used in this naive implementation for filtering for next generation but is part of the interface.
        if task_id:
            logger.debug(f"InMemoryDatabaseAgent.get_programs_for_next_generation called with task_id='{task_id}', but this implementation does not currently filter by task_id for this operation.")
...
if __name__ == "__main__":
    ...
    async def test_db():
        ...
        # Test with task_id (it will be ignored by this implementation but should not error)
        best_correctness = await db.get_best_programs(task_id="dummy_task_id", limit=2, objective="correctness_score", sort_order="desc")
        ...
        best_runtime_asc = await db.get_best_programs(task_id="dummy_task_id", limit=2, objective="runtime_ms", sort_order="asc")
        ...
        best_runtime_desc = await db.get_best_programs(task_id="dummy_task_id", limit=2, objective="runtime_ms", sort_order="desc")
